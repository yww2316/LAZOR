...
class bffTest(unittest.TestCase):
    def setUp(self):
        ...
        self.parseddata=[[2, 3]], 3, 3, 0, [[3, 6, -1, -1]], ['B o o', 'o o o', 'o o o']
        self.reflect=(-1,1)
        self.opaque=(0,0)
    ...
    def test_Check_Reflect(self):
        '''
        Checks to make sure that the reflect block outputs the correct velocities.
        '''
        self.assertEqual(Block.Reflect(1,1,1), self.reflect,'The reflect block information is not saved correctly.' )

    def test_Check_Opaque(self):
        '''
        Checks to make sure that the opaque block outputs the 0 velocity.
        '''
        self.assertEqual(Block.Opaque(1,1,1), self.opaque,'The opaque block information is not saved correctly.' )

    def test_Check_Refract(self):
        '''
        Checks to make sure that the refract block outputs the 0 velocity.
        '''
        self.assertEqual(Block.Refract(1,1,1), self.refract,'The refract block information is not saved correctly.' )
    def test_Block_call(self):
        '''
        Checks to ensure that the correct velocities are outputted when a specific block is called.
        '''
        bah=Block('B',1,2,1)
        self.assertEqual(bah('A',1,1,1), self.block,'The block information is not saved correctly.' )
    def test_Solve(self):
        '''
        Checks to make sure that the correct solution is outputted by the solve function.
        '''
